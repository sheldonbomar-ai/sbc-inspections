      ...
      <div data-content="" style={{flex:1,overflow:"auto",paddingBottom:mob?70:0}}><div style={{padding:mob?"14px 12px":"20px 32px"}}>
        ...
        {pg==="sheet"&&!selI&&(()=>{
          ...
          return <>
            ...
            {projWithRev.length>0&&<><div data-pend-header="" style={{background:C.orb,padding:"7px 14px",borderTop:`2px solid ${C.or}`,marginTop:6}}><span style={{fontSize:11,fontWeight:700,color:C.or}}>↺ REVISIONS ({projWithRev.reduce((n,p)=>n+(p.revisions||[]).filter(r=>r.status==="Open").length,0)})</span></div>{projWithRev.map(p=>{const prev=(p.revisions||[]).filter(r=>r.status==="Open");return(
              <div data-insp-row="" key={"rev-"+p.id} onClick={()=>{sSP(p.id);setPg("projects");}} style={{padding:"8px 14px",background:C.orb,borderBottom:`1px solid ${C.bd}`,cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><span data-name="" style={{fontSize:12,fontWeight:600,textTransform:"uppercase"}}>{p.clientName}</span><div style={{display:"flex",alignItems:"center",gap:4}}><div data-status-dot="" style={{width:6,height:6,borderRadius:"50%",background:C.or}}/><span data-result="" style={{fontSize:10,color:C.or}}>Revision Required</span></div></div>
                {prev.map(r=><div key={r.id} data-type="" style={{fontSize:11,color:C.or,fontWeight:600}}>{r.description}{r.inspection?` (${r.inspection})`:""}</div>)}
                <div data-detail="" style={{fontSize:10,color:C.w3,marginTop:2}}>{p.city}{p.address&&p.address!=="TBD"?` · ${p.address}`:""}</div>
              </div>);})}</>}
            {projWithCO.length>0&&<><div data-pend-header="" style={{background:C.rdb,padding:"7px 14px",borderTop:`2px solid ${C.rd}`,marginTop:6}}><span style={{fontSize:11,fontWeight:700,color:C.rd}}>$ CHANGE ORDERS ({projWithCO.reduce((n,p)=>n+(p.changeOrders||[]).filter(co=>co.status==="Pending").length,0)})</span></div>{projWithCO.map(p=>{const pco=(p.changeOrders||[]).filter(co=>co.status==="Pending");return(
              <div data-insp-row="" key={"co-"+p.id} onClick={()=>{sSP(p.id);setPg("projects");}} style={{padding:"8px 14px",background:C.rdb,borderBottom:`1px solid ${C.bd}`,cursor:"pointer"}}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}><span data-name="" style={{fontSize:12,fontWeight:600,textTransform:"uppercase"}}>{p.clientName}</span><div style={{display:"flex",alignItems:"center",gap:4}}><div data-status-dot="" style={{width:6,height:6,borderRadius:"50%",background:C.rd}}/><span data-result="" style={{fontSize:10,color:C.rd}}>Change Order</span></div></div>
                {pco.map(co=><div key={co.id} data-type="" style={{fontSize:11,color:C.rd,fontWeight:600}}>{co.description}{co.cost?` ($${co.cost})`:""}</div>)}
                <div data-detail="" style={{fontSize:10,color:C.w3,marginTop:2}}>{p.city}{p.address&&p.address!=="TBD"?` · ${p.address}`:""}</div>
              </div>);})}</>}
