        ...
        {pg==="projects"&&selP&&(()=>{const p=proj.find(x=>x.id===selP);if(!p)return null;const pi=insp.filter(i=>i.projectId===selP);const addNote=(txt)=>setP(v=>v.map(x=>x.id===selP?{...x,comments:[...(x.comments||[]),{id:uid(),text:txt,author:user.displayName||user.email,date:td(),time:new Date().toLocaleTimeString("en-US",{hour:"numeric",minute:"2-digit"})}]}:x));return <>
          ...
          <div style={S.cd}><h4 style={{fontSize:13,fontWeight:700,marginBottom:10,display:"flex",alignItems:"center",gap:8}}><span>💬</span>Notes<span style={{fontSize:10,fontWeight:500,color:C.w3}}>({(p.comments||[]).length})</span></h4>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10,maxHeight:380,overflowY:"auto"}}>
              {(p.comments||[]).map(c=>{const mine=(c.author||"")===(user.displayName||user.email);const who=c.author||"Unknown";return <div key={c.id} style={{display:"flex",flexDirection:"column",alignItems:mine?"flex-end":"flex-start"}}>
                <div style={{fontSize:10,color:C.w3,marginBottom:2,padding:"0 4px"}}><b style={{color:mine?C.bl:C.gr}}>{who}</b> · {fmt(c.date)}{c.time?" · "+c.time:""}</div>
